...
dataset = ImageFolder("dataset/64x64/", transform=transform)
#debug_subset = Subset(dataset, list(range(1000)))
train_loader = DataLoader(
    ...
    shuffle=True,
    drop_last=True,
    num_workers=0,              # macOS + lambda/Spawn 兼容：先用 0，确认没问题再调大
...
# ===============================
# UNet (修正版：精确匹配每次 concat 的 skip 通道数)
# ===============================
...
# ===============================
# Training / Sampling Utils
# ===============================
def update_ema(ema_model, model, ema=0.999):
    with torch.no_grad():
        for p_ema, p in zip(ema_model.parameters(), model.parameters()):
            p_ema.data.mul_(ema).add_(p.data, alpha=1 - ema)
...
def train():
    for epoch in range(num_epochs):
        ...
        for images, _ in tqdm(train_loader,
            ...
            b = images.size(0)
            t = torch.randint(0, total_timesteps, (b,), device=device).long()
            noise = torch.randn_like(images)
            images_t = gdf_util.q_sample(images, t, noise)
            pred_noise = network(images_t, t)
            loss = criterion(pred_noise, noise)

            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
            update_ema(ema_network, network)
        ...
        # ---- Checkpoint paths ----
        CKPT_DIR = "models/epoch_%d" % (epoch + 1)
        NET_CKPT = os.path.join(CKPT_DIR, "64x64_unet.pth")
        EMA_CKPT = os.path.join(CKPT_DIR, "64x64_unet_ema.pth")
        # ---- 保存当前权重（原网 + EMA 网）----
        # 也可以隔几个 epoch 再存，防止频繁 IO
        save_model(network, NET_CKPT)
        save_model(ema_network, EMA_CKPT)
...
if __name__ == "__main__":
    ...
    # 若已有 EMA 权重可直接采样；否则也能用随机权重采样（只是噪声）
    epoch_check = 45
    CKPT_DIR = "models/epoch_%d" % (epoch_check)
    NET_CKPT = os.path.join(CKPT_DIR, "64x64_unet.pth")
    EMA_CKPT = os.path.join(CKPT_DIR, "64x64_unet_ema.pth")
    sample_and_plot(ckpt_path = EMA_CKPT, num_rows=8, num_cols=8)
